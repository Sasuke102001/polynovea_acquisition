"""
029_load_magicpin_lower_step6_fitness.py
Loads fitness dimensions from the MagicPin LOWER BLOCK (online reviews)
step_6_output.json into:
  - venue_fitness_dimensions  (source='magicpin_lower')
  - behavioral_summary        (source='magicpin_lower')
  - intervention_triggers     (source='magicpin_lower')

MagicPin provides two distinct data blocks for each venue:
  Upper block = static metadata (menu, photos, venue details) → source='magicpin_upper'
  Lower block = online reviews, ratings over time             → source='magicpin_lower'

These are separate evidence layers. Both compound in the DB alongside
source='google', source='zomato', etc. The blend runner (027) averages
all present sources equally per venue.

Data path: Database/data/raw/magicpin_lower/<region>/step_6_output.json
  (place lower block BIF output files here before running)

Join: place_id → venues.id
Run after: 018_add_source_columns.py, 028_migrate_magicpin_source_names.py
Then run: 027_blend_fitness.py to recompute source='blended'
"""
...
BASE_PATH        = os.path.join(os.path.dirname(__file__), '..', 'data', 'raw', 'magicpin_lower')
REGIONS          = ['mumbai', 'navi-mumbai', 'sobo', 'thane']
SOURCE           = 'magicpin_lower'
PIPELINE_VERSION = 'magicpin-lower-bif-1.0'
...
FITNESS_SQL = f"""
    ...
    VALUES (%s, '{SOURCE}', %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, 1)
...
INTERVENTION_SQL = f"""
    ...
    VALUES (%s, '{SOURCE}', %s, %s, %s, %s, %s, %s, %s, %s)
...
def fd_details(fd):
    dims = ('fitness_for_office_lunch', 'fitness_for_repeat_habit',
            'fitness_for_social_dwell', 'fitness_for_group_energy',
            'fitness_for_destination_visit')
    return json.dumps({
        d: {
...
def main():
    print(f"\n029_load_magicpin_lower_step6_fitness.py — MagicPin lower block → source='{SOURCE}'\n")
    ...
    try:
        ...
        total_loaded = sum(r['loaded'] for r in summary)
        print(f"{'='*55}")
        print(f"  COMPLETE")
        print(f"  Venues     : {total_loaded:,}  (source='{SOURCE}')")
        print(f"  Interventions: {sum(r['interventions'] for r in summary):,}")
        print(f"\n  Next step: run 027_blend_fitness.py to recompute source='blended'")
        print(f"{'='*55}\n")
