...
Sources handled:
  ...
  magicpin_lower — same, from data/raw/magicpin_lower/ (when data arrives)

Run before:
  005_load_similarity.py          (google)
  025_load_magicpin_step5b.py     (magicpin_upper)
  and equivalent lower-block loader (magicpin_lower)
...
SOURCES = [
    ...
    {
        'name':        'magicpin_lower',
        'base_path':   os.path.join(SCRIPTS_DIR, '..', 'data', 'raw', 'magicpin_lower'),
        'regions':     ['mumbai', 'navi-mumbai', 'sobo', 'thane'],
        'lookup_file': 'step_3_signals_extracted.json',
        'lookup_key':  'venues',
        'name_field':  'name',      # same field convention as upper block
        'id_field':    'place_id',
    },
]
